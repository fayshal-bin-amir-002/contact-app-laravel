    ...
    <td width="150">
        @if ($showTrashButtons)
            <form action="{{ route("contacts.restore", $contact->id) }}" method="POST" style="display: inline">
                ...
                <form action="{{ route("contacts.force-delete", $contact->id) }}" onsubmit="return confirm('Your data will be removed permanenetly!')" method="POST" style="display: inline">
                @csrf
                @method("delete")
                <button type="submit" class="btn btn-sm btn-circle btn-outline-danger" title="Force Delete"><i class="fa fa-times"></i></button>
            </form>
        @else
